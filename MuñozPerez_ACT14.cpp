...
void guardar(){
	...
	if(archivo.is_open()){
		for (int x=0; x <6; x++){
			archivo <<"NOMBRE: "<< persona[x].nombre<< persona[x].apeido<<
			", LUGAR:"<<persona[x].municipio<<", "<<persona[x].ciudad<<", "<<persona[x].estado<<
			", EDAD: "<<persona[x].edad<<", ESTATURA: "<<persona[x].estatura<<"\n";
		}//fin del for
		archivo.close();
		color(2);
